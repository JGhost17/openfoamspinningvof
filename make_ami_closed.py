...
# Generate a CLOSED cylindrical AMI surface (top disk + bottom disk + side wall)
# that fully encloses impeller.STL with margin on all sides.
...
for i in range(N_SEG):
    ...
    facets.append(((0,0,1), (CX,CY,Z_HIGH), (x0,y0,Z_HIGH), (x1,y1,Z_HIGH)))
...
for i in range(N_SEG):
    ...
    x0, y0 = CX+R*math.cos(a0), CY+R*math.sin(a0)
    x1, y1 = CX+R*math.cos(a1), CY+R*math.sin(a1)
    facets.append(((0,0,-1), (x1,y1,Z_LOW), (x0,y0,Z_LOW), (CX,CY,Z_LOW)))
...
print('Wrote CLOSED ami cylinder: centre (%.0f,%.0f), R=%.0f mm, z=[%.0f,%.0f] mm, %d facets'
